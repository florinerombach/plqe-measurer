...
def main():
    ...
    #enter parameters
    input('This program takes PLQE measurements with nonlinearity correction, NO stray light correction, and common backgrounds. Please make sure the laser is set to the desired power and ShutterControl is closed. Press enter to continue.')
    ...
    def measurespot(samplename, foldername, longIT_ms, shortIT_ms, spot_num, port_object, shutter_wait=1):
        ...
        #run analysis
        analysisprogram = 'python D:/PLQY/PLQE_bw.py --ignore-gooey -st '+str(shortIT_ms)+' -lt '+str(longIT_ms)+' -lr '+str(laserrange)+' -plr '+str(plrange)+' -sp '+str(directory)+'\\'+str(samplename)+'_short_in.txt'+' -lp '+str(directory)+'\\'+str(samplename)+'_long_in.txt'+' -c -cl'
